...
app.get('/produtos/:id', function(req, res) {
    if (req.session.user) {
        ...
        const queryPrimeiraPalavra = `
            SELECT SUBSTRING_INDEX(nome_produto, ' ', 1) AS primeira_palavra, foto
            FROM produtos
            WHERE id = ?`;
            ...
            if (error) {
                ...
                connection.query(queryProdutosSemelhantes, [primeiraPalavra, cidadeDoUsuario], (error, results) => {
                    if (error) {
                        ...
                        // Convertendo as fotos dos produtos para base64
                        const produtosSemelhantes = results.map((produto) => {
                            const fotoBase64 = Buffer.from(produto.foto).toString('base64');
                            return {
                                ...produto,
                                foto: fotoBase64,
                            };
                        ...
                        connection.query(queryProdutoAtual, [produtoId, cidadeDoUsuario], (error, results) => {
                            if (error) {
                                ...
                                const fotoAtualBase64 = Buffer.from(produtoAtual.foto).toString('base64');
                                produtoAtual.foto = fotoAtualBase64;
